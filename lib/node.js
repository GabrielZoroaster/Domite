...
class NodeGenerator {
	...
	* list(){
	...
	take(limit){
		return new NodeIterator(Take(this.list(), limit));
	}

	drop(limit){
		return new NodeIterator(Drop(this.list(), limit));
	}

	filter(cb){
		return new NodeIterator(Filter(this.list(), cb));
	}
	...
	reduce(cb, ... args){
		return Reduce(this.list(), cb, ... args);
	}
