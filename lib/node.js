...
function WindowRect({left, top, width, height}){
	return new DOMRect(left + window.scrollX, top + window.srollY, width, height);
}
...
class Node {
	...
	get sr(){
		return new ScrollRect(this.tag);
	}
...
class OffsetRect extends Rect {
	...
	get parent(){
		return Node.from(this.tag.offsetParent);
	}
